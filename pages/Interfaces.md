...
Одним из основным принципов TypeScript является то, что проверка типов основывается на *форме* значений.
Этот подход иногда называется "утиной типизацией" либо "структурным подтипированием".
В TypeScript интерфейсы выполняют функцию именования этих типов, и являются мощным способом определения соглашений внутри кода, а также за пределами проекта.
...
Эта функция принимает один параметр, который требует, чтоб у переданного объекта было свойство под именем `label`, которое имело бы строковый тип.
...
Мы можем переписать этот пример, на этот раз используя интерфейс для того, чтоб отразить необходимость наличия свойства `label` строкового типа:
...
Интерфейс `LabelledValue` — это имя, которое теперь можно использовать, чтоб задать требование из предыдущего примера.
...
Стоит отметить, что проверка типов не требует, чтоб свойства шли в определенном порядке: важно лишь, что необходимые свойства присутствуют и имеют необходимый тип.
...
Такие необязательные (опциональные) свойства популярны, к примеру, при передаче в функцию аргументов в виде объекта, в котором указаны всего несколько свойств.
...
Интерфейсы с необязательными свойствами записываются подобно другим интерфейсам, но каждое опциональное свойство помечается символом `?` в конце имени.
...
К примеру, если бы мы ошиблись при вводе имени свойства `color` в `createSquare`, то получили бы сообщение об ошибке, информирующее об этом:
...
Вы можете указать это, добавив `readonly` перед именем свойства:
...
Эта проверка делается когда литералы либо присваиваются другим переменным, либо передаеются в качестве аргументов.
...
Это значит, что когда вы сталкиваетесь с проблемами, которые порождает такая проверка (к примеру, при передаче объекта с аргументами функции), возможно, придется изменить объявления типов.
В данном случае, если передача объекта, в котором могут быть одновременно свойства `color` и `colour`, приемлема, нужно исправить определение `SquareConfig`, чтоб отразить это.
...
Для того, чтоб описать функцию с помощью интерфейса, к нему добавляют сигнатуру вызова.
...
Имена параметров не обязательно должны совпадать, чтоб функция прошла проверку на соответствие типов.
...
Если бы функция возвращала числа или строки, то компилятор во время проверки типов предупредил бы о том, что тип возвращаемого значения не совпадает с типом, указанным в интерфейсе `SearchFunc`.
...
Аналогично тому, как интерфейсы используются для описания функций, можно описать типы так, чтоб с ними можно было использовать оператор индекса — например, вот так `a[10]` или так `ageMap["daniel"]`.
...
Кроме того, что строковые индексы — мощный способ описания словарей, они диктуют требование того, чтоб типы всех свойств соответствовали типу, который возвращает операция индекса.
...
Кроме того, сигнатуру индекса можно сделать доступной только для чтения, чтоб запретить присваивание индексам:
...
В таких языках, как C# и Java интерфейсы наиболее часто используются для того, чтоб явно указать, что класс соответствует определенному соглашению. Это возможно и в TypeScript.
...
Интерфейсы описывают публичную часть класса, но не публичную и приватную одновременно.
...
Как говорилось ранее, интерфейсы могут описывать сложные типы, которые встречаются в "боевом" JavaScript.
...
При взаимодействии со сторонним кодом может понадобиться использовать подобные приемы, чтоб полноценно описать тип.

# Интерфейсы, расширяющие классы

Когда интерфейс расширяет класс, интерфейс наследует члены класса, но не их реализацию.
Это аналогично тому, если бы интерфейс описывал все члены класса, но не указывал их реализацию.
...
Такие подклассы не обязаны иметь отношение друг к другу, кроме того, что они наследуются от одного базового класса.
К примеру:
...
Так будет потому, что для совместимости приватных членов необходимо, чтоб они были объявлены в одном и том же базовом классе, а это возможно лишь для наследников `Control`.
