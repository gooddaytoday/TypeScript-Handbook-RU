...
*Декоратор* -- это особый вид объявления, который может быть прикреплён к [объявлению класса](#class-decorators), [методу](#method-decorators), [ацессору](#accessor-decorators), [свойству](#property-decorators) или [параметру](#parameter-decorators).
Декораторы используют форму `@выражение`, где `выражение` должно вычислиться в функцию, которая будет вызвана во время исполнения, вместе с информацией декорированного объявления.
...
Смотрите раздел [Метаданные](#metadata) для большей информации о библиотеке `reflect-metadata`.
