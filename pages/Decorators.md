...
*Декоратор* -- это особый вид объявления, который может быть прикреплён к [объявлению класса](#class-decorators), [методу](#method-decorators), [ацессору](#accessor-decorators), [свойству](#propery-decorators) или [параметру](#parameter-decorators).
Декораторы используют форму `@выражение`, где `выражение` должно вычислиться в функцию, которая будет вызвана во время исполнения, вместе с информацией декорированного объявлении.
...
> ЗАМЕТКА&emsp; Увидеть более подробный пример декоратора можно в разделе [Декораторы Класса](#class-decorators).

## Фабрика Декораторов

Если мы захотим настроить то, как декоратор применятеся к объявлению, мы можем написать фабрику декораторов.
*Фабрика Декораторов* -- это функция, возвращающая выражение, которая будет вызвана декоратором во время исполнения программы.

Мы можем написать фабрику декораторов следющим образом:

```ts
function color(value: string) { // это фабрика декораторов
    return function (target) { // это декоратор
        // сделать что-то с перменными 'target' и 'value'...
    }
}
```

> ЗАМЕТКА&emsp; Более подробный пример фабрики декораторов можно увидеть в разделе [Декораторы Методов](#method-decorators).
...
Если мы будем использовать [фабрику декораторов](#decorator-factories), мы можем наблюдать этот порядок вычисления вместе с следующим примером:
...
1. *Декораторы Параметров* с последующими *Декораторами Методов*, *Ацессоров* или *Свойств* применяются для каждого члена экземпляра.
2. *Декораторы Параметров* с последующими *Декораторами Методов*, *Ацессоров* или *Свойств* применяются для каждого статического члена.
...
> ЗАМЕТКА&emsp; *Дескриптор Свойства* будет `undefined`, если цель Вашего скрипта меньше, чем `ES5`.

Если декоратор метода возвратит значение, это значение будет использоваться как *Дескриптор Свойства* для метода.

> ЗАМЕТКА&emsp; Возвращаемое значение игнорируется, если цель вашего скрипта меньше, чем `ES5`.
...
Декоратор `@enumerable(false)` -- это [фабричный декоратор](#decorator-factories).
Когда будет вызван декоратор `@enumerable(false)`, он изменит свойство `enumerable` дескриптора свойства.

## Декоратор Ацессора

*Декоратор Ацессора* объявляется перед объявлением ацессора.
Декоратор ацессора применяется к *Дескриптору Свойства* и может быть использован для наблюдения, модифицирования или замещения определения ацессора.
Декоратор ацессора не может быть использован в файле объявления или в любом другом окружающем контексте (например, класс `declare`).

> ЗАМЕТКА&emsp; TypeScript запрещает декорирование `get` и `set` ацессоров для одного члена.
Вместо этого, все декораторы члена должны быть применены к первому ацессору, указанному в текстовом порядке.
...
Выражение для декоратора ацессора будет вызвано как функция во время исполнения со следующими тремя аргументами:
...
Если декоратор ацессора возвратит значение, это значение будет использоваться как *Дексриптор Свойства* для члена.

> ЗАМЕТКА&emsp; Возвращаемое значение игнорируется, если цель вашего скрипта меньше, чем `ES5`.

Дальше -- пример декоратора ацессора (`@configurable`), применённый к члену класса `Point`:
...
Декоратор свойства не может быть использован в файле объявления или в любом другом окружающем контекстке (например, в классе `declare`).
...
Декоратор `@format("Привет, %s")` [фабричный декоратор](#decorator-factories).
...
Смотрите раздел [Метаданные](#metadata) для большей информации о библотеке `reflect-metadata`.

## Декораторы Параметров

*Декоратор Параметра* объявляется перед объявлением параметра.
Декоратор параметра применяется к объявлению конструктора или метода класса.
