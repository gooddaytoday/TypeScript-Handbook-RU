...
К примеру, может понадобиться модуль, который вызывается с `new` или без, создавая при этом различные типы, имеет различные именованные типы, упорядоченные иерархически, и несколько свойств на самом объекте модуля.

Прочитав это руководство, вы получите инструменты для написания сложных файлов определений, предоставляющих удобный API.
Это руководство фокусируется на модульных (или UMD) библиотеках из-за множества их вариантов и большего разнообразия.

## Ключевые принципы

Для того, чтобы полноценно понять, как придать определению любую форму, нужно понять ключевые принципы работы TypeScript.
...
* Объявления `import`, который ссылается на тип

Каждое из таких объявлений создает новое имя типа.

### Значения

Как и с типами, вы, скорее всего, понимаете, что такое значение.
...
Посмотрим на полезные стороны такого поведения.

### Встроенные комбинации

Проницательный читатель заметит, что, например, `class` появляется и в списке *типов*, и в списке *значений*.
...
Для этого можно использовать комбинирование, представив две различных сущности (значение и тип) под одним именем `Bar`:
...
Некоторые типы объявлений могут комбинироваться между несколькими объявлениями.
...
Объявление `namespace` можно использовать для добавления новых типов, значений и пространств имен любым способом, который не создает конфликтов.
...
Также к классу можно добавить тип, заключаемый в пространство имен:
...
Второй блок создает следующие сущности:
